...
public class EnviarDocumentos {
    ...
    public void enviarDatosServidor(){
        ...
        //new EnviarDatosServidorVC().execute();
    }
    public void enviarDatosServidorVolanteo(){
        //new EnviarDatosServidor().execute();
        new EnviarDatosServidorVC().execute();
    }
    private class EnviarDatosServidor extends AsyncTask<String,Integer,String> {
        ...
        protected void onPostExecute(String s) {
            ...
            //Se envia los que estan en modo VOlanteo
            new EnviarDatosServidorVC().execute();
            //Toast.makeText(contexto,"Iniciando Modo Volanteo...",Toast.LENGTH_LONG).show();
    ...
    * pero de los documentos que no fueron sincronizados ni asignados
    * y se insertaron de manera manual desde el celular
     */
    public class EnviarDatosServidorVC extends AsyncTask<String,Integer,String> {
        ...
        protected String doInBackground(String... strings) {
            ...
            */
            ...
            //Primero se recupera el listado de fotografias sel BD offline para enviar.
            ArrayList<ArrayList> listadoDocEntregado=db.ListarDocHechosOffLineXSuministro();
            ...
            for (int i = 0; i<listadoDocEntregado.size(); i++){
                String CodigoDocInternoOffline=listadoDocEntregado.get(i).get(0).toString();
                String NroSuministro=listadoDocEntregado.get(i).get(1).toString();
                String FechaVisita=listadoDocEntregado.get(i).get(2).toString();
                //String EstadoEntrega=listadoDocEntregado.get(i).get(3).toString();
                String VisitaLatitud=listadoDocEntregado.get(i).get(4).toString();
                String VisitaLongitud=listadoDocEntregado.get(i).get(5).toString();
                //String EstadoEnvioServidor=listadoDocEntregado.get(i).get(6).toString();
                ...
                try {
                    ...
                    try {
                        ...
                        multipart.addFormField("usuario", "" + usuario);
                        multipart.addFormField("password", "" + password);
                        multipart.addFormField("NroSuministro", ""+NroSuministro);
                        multipart.addFormField("LatitudVisita", ""+VisitaLatitud);
                        multipart.addFormField("LongitudVisita", ""+VisitaLongitud);
                        multipart.addFormField("FechaEjecutado", ""+FechaVisita);
                        multipart.addFormField("Observaciones", "Modo Volanteo");
                ...
                //String Estadorespuesta=LeerXml(""+xmlTexto);
                String Estadorespuesta=respuestaparaaccion;
                int CodigoDocumentito = Integer.parseInt(CodigoDocInternoOffline);
                if (Integer.parseInt(Estadorespuesta)>0){
                    publishProgress(i);
                    response=i+""+Estadorespuesta;
                    //Envia las fotos a servidor
                    if (IdObtenidoServidor>0){
                        ...
                        //new Enviarfotos().execute(IdObtenidoServidor+"",CodigoDocumentito+"");//CodigodevuelltServidir, CodigoDoc
                        new EnviarfotosVolanteo().execute(IdObtenidoServidor+"",NroSuministro);//CodigodevuelltServidir, CodigoDoc
                    ...
                    databaseoffline.updateDocumentoXSuministro(
                            CodigoDocumentito,
                            "1"//1= Enviado
                    );
                }else if (Estadorespuesta.equals("-2")){//problemas de lectura y escritura en android
        ...
        protected void onPostExecute(String s) {
            ...
            if(Integer.parseInt(s)>0){
                ...
                Toast.makeText(contexto,"No hay Fotos para enviar("+s+")",Toast.LENGTH_LONG).show();
            }
        }
    }
    private class EnviarfotosVolanteo extends AsyncTask<String,Integer,String>{
        ...
        protected String doInBackground(String... strings) {
            ...
            String respuesta=sa.postArchivo();
            */
            //Generalidades gen=(Generalidades)contexto;
            String charset = "UTF-8";
            String requestURL = RutaFotos;// gen.getCadena()+"webservices/guardarfoto.php";//?Idfotovisitascampo="+IdObtenidoServidor;//,strings[1];*/
                    ...
                    if (Integer.parseInt(strings[0])>0){
                        ...
                        if (CantidadFotosModificadas>0){//se copuerba que se hizo el cambio
                            ...
                            if (respuestaServidor.equals("si")){//si el servidor responde "si"
                                ...
                                if ((CantidadRegistradosOnline>0)){
                                    //Foto online(archivo y BD) y Offline(BD)
                                    //response=(Integer.parseInt(response)+1)+"";//"3";
                                    response = (response==null)?"3":(Integer.parseInt(response)+1)+"";
                                }
                            }else{
                                int CantidadRegistradosOnline=db.updateFotosEstado(Integer.parseInt(CodigoFotos),"0");//0=Foto no enviado
                                if ((CantidadRegistradosOnline>0)){
        ...
        protected void onPostExecute(String s) {
            ...
            if (!(Integer.parseInt(s)>=3)){
                Toast.makeText(contexto,"No se envio FV (rpta: "+subirfoto+")",Toast.LENGTH_LONG).show();
            }
